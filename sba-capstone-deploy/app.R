...
server <- function(input, output, session) {
  ...
  observe({
    updateSelectInput(session, "parent_agency", choices = c("All", sort(unique(smcon$parent_agency))))
    updateSelectInput(session, "naics_group", choices = c("All", sort(unique(smcon$naics_group))))
    updateSliderInput(session, "fiscal_year", min = min(smcon$fiscal_year), max = max(smcon$fiscal_year), value = min(smcon$fiscal_year))
  })
    ...
    counties_sf %>%
      left_join(
        smcon_filtered() %>%
          ...
          summarize(state = first(state), total_obligation = sum(total_obligation, na.rm = TRUE), .groups = "drop"),
  ...
  observe({
    ...
    breaks <- if (all(vals == 0 | is.na(vals))) {
      ...
      unique(c(0, 1, quantile(vals[vals > 0], probs = seq(0.2, 1, by = 0.2), na.rm = TRUE)))
    }
    
    pal <- colorBin("YlGnBu", domain = vals, bins = breaks, na.color = "#f0f0f0", right = FALSE)
    ...
    if (input$view_mode == "State") {
      proxy %>%
        addPolygons(
          ...
          label = ~paste0(state_abbr, ": $", formatC(total_obligation, big.mark = ",", format = "f", digits = 0)),
          highlightOptions = highlightOptions(color = "black", weight = 2)
        ) %>%
        addLegend("bottomright", pal = pal, values = vals, title = "State Obligations")
      ...
      proxy %>%
        addPolygons(
          ...
          label = ~paste0(NAME, ": $", formatC(total_obligation, big.mark = ",", format = "f", digits = 0))
        ) %>%
        addLegend("bottomright", pal = pal, values = vals, title = "County Obligations")
    }
  })
